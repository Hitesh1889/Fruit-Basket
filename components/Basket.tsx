...
export const Basket: React.FC<BasketProps> = ({ basket, isSelected, isValidTarget, onClick }) => {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center justify-end h-80 md:h-[26rem] lg:h-[30rem] w-28 md:w-36 lg:w-40 relative mx-2 md:mx-4 group"
    >
      ...
      <motion.button
        ...
        className={`
          w-full h-full flex flex-col justify-end items-center pb-4
          transition-all duration-300 rounded-b-[3rem] rounded-t-2xl
      ...
      >
          ...
          {/* Fruits Stack */}
          <div className="flex flex-col-reverse items-center justify-start w-full mb-8 z-10 pb-4">
          ...
          {isSelected && (
             <motion.div 
               ...
               className="absolute -top-16 text-5xl filter drop-shadow-lg animate-bounce"
             >
               👇
             </motion.div>
          )}
          ...
          {isSolved && (
             <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute -bottom-10 bg-yellow-400 text-yellow-900 text-sm font-black px-4 py-2 rounded-full shadow-xl z-30 tracking-wider border-2 border-yellow-600"
             >
               MATCH!
             </motion.div>
          )}
